        ...
        (snapshot) => {
          if (!snapshot.empty) {
            ...
            snapshot.forEach((doc) => {
              list.push({ id: doc.id, ...(doc.data() as Omit<RideSchedule, "id">) });
            });

            // Filter H-5: hanya jadwal yang berjarak 0 sampai 5 hari dari hari ini (H-0 s/d H-5)
            const eligibleSchedules = list
              .map((s) => ({
                schedule: s,
                diffDays: getDaysDifference(s.date),
              }))
              .filter((item) => item.diffDays >= 0 && item.diffDays <= 5)
              .sort((a, b) => a.diffDays - b.diffDays); // Urutkan yang paling dekat

            // Jika ada jadwal dalam H-5, ambil yang terdekat
            ...
            // Jika kosong di database, jangan tampilkan apa-apa
            setSchedule(null);
          }
        },
        (error) => {
          console.warn("Error membaca jadwal:", error);
          setSchedule(null);
        }
      );
    } catch {
            ...
            <div className="p-6 sm:p-8 space-y-5">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-2">
                ...
                <div className="bg-black/50 p-3.5 rounded-xl border border-white/10 space-y-1">
                  ...
                  <p className="text-sm font-bold text-[#EAE6DD]">{schedule.time || "05:30 WIB"}</p>
                </div>
                ...
                <div className="bg-black/50 p-3.5 rounded-xl border border-white/10 space-y-1 sm:col-span-2">
                  ...
                  <p className="text-sm font-bold text-white">{schedule.meetingPoint}</p>
                </div>
                ...
                {schedule.distance && (
                  <div className="bg-black/50 p-3.5 rounded-xl border border-white/10 space-y-1">
                    ...
                    <p className="text-sm font-bold text-[#C44341]">{schedule.distance}</p>
                  </div>
                )}
                ...
                <div className="bg-black/50 p-3.5 rounded-xl border border-white/10 space-y-1">
                  ...
                  <p className="text-xs font-semibold text-gray-200">
                    {schedule.bikePolicy || "Semua jenis sepeda boleh"}
                  </p>
                </div>
              </div>
              ...
              {schedule.notes && (
                <div className="p-3 bg-[#C44341]/10 rounded-lg border border-[#C44341]/25 text-xs text-[#EAE6DD] flex items-start gap-2.5">
                  ...
                  <span className="leading-relaxed">{schedule.notes}</span>
                </div>
              )}
              ...
              <div className="pt-2 flex flex-col sm:flex-row gap-3">
                <a
                  href={schedule.joinUrl || "https://chat.whatsapp.com/Iec3x7imtTKI9DMXaJzP9z"}
                  target="_blank"
